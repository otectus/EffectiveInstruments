...
public final class ArsNouveauEventHandler {
    ...
    public static void onEntityJoinLevel(EntityJoinLevelEvent event) {
        ...
        if (!ArsNouveauReflection.isStarbuncle(mob)) return;
        try {
            mob.goalSelector.addGoal(GOAL_PRIORITY, new StarbuncleInstrumentGoal(mob));
        } catch (Throwable ignored) {}
    }

    @SubscribeEvent
    public static void onLivingDeath(LivingDeathEvent event) {
        if (!ArsNouveauReflection.isStarbuncle(event.getEntity())) return;
        ImmersiveMelodiesAuraHandler.unregisterActiveMobileNpc(event.getEntity().getUUID());
    }

    private ArsNouveauEventHandler() {}
}
