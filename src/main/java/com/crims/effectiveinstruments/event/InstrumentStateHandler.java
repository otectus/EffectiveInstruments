...
/**
 * Backend-agnostic Forge lifecycle hooks. The previously-included Genshin-
 * specific {@code InstrumentOpenStateChangedEvent} handler moved to
 * {@code compat.genshin.GenshinInstrumentEventHandler} in 1.5.0 — it is
 * registered manually only when GI is installed. This class no longer
 * imports any backend mod's classes.
 */
...
public class InstrumentStateHandler {
    ...
    public static void onPlayerLogout(PlayerEvent.PlayerLoggedOutEvent event) {
        ...
        StationaryInstrumentNoteService.onPlayerLogout(event.getEntity().getUUID());
    }
